...
for prompt_index in 2 6; do
  output="$BASE/f_module_propagation_prompt${prompt_index}_seed42_gpu6"
  mkdir -p "$output"
  python tools/profile_module_propagation.py \
    ...
    --module-groups ffn,late_blocks,spatial_attention \
    ...
    --output-dir "logs_fp16_flash/stage_numeric_profiling/f_module_propagation_prompt${prompt_index}_seed42_gpu6" \
