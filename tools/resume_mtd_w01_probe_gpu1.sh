...
GPU_ID=1
RESUME_STATE="$OUT/calibration/reconstruction_state_iter_00005500.pth"

mkdir -p "$OUT"

fail() {
    echo "FAILED gpu=$GPU_ID $(date '+%F %T')" | tee "$OUT/status.txt"
    exit 1
}
trap fail ERR

[[ -s "$RESUME_STATE" ]] || {
    echo "Missing strict-resume state: $RESUME_STATE" >&2
    exit 1
}

GPU_UUID=$(nvidia-smi --id="$GPU_ID" --query-gpu=uuid --format=csv,noheader)
echo "RESUME_WAITING gpu=$GPU_ID from=5500 uuid=$GPU_UUID $(date '+%F %T')" | tee "$OUT/status.txt"
idle_checks=0
while (( idle_checks < 6 )); do
    ...
    if grep -Fxq "$GPU_UUID" <<<"$compute_uuids"; then
        idle_checks=0
        echo "RESUME_WAITING gpu=$GPU_ID busy from=5500 $(date '+%F %T')" > "$OUT/status.txt"
    else
        ((idle_checks += 1))
        echo "RESUME_WAITING gpu=$GPU_ID idle_check=$idle_checks/6 from=5500 $(date '+%F %T')" > "$OUT/status.txt"
    fi
    sleep 10
done

echo "RUNNING gpu=$GPU_ID resumed_from=5500 $(date '+%F %T')" | tee "$OUT/status.txt"
...
echo "COMPLETED gpu=$GPU_ID resumed_from=5500 $(date '+%F %T')" | tee "$OUT/status.txt"
