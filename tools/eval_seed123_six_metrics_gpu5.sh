...
wait_for_gpu() {
    local uuid idle_checks=0
    uuid=$(gpu_uuid)
    [[ -n "$uuid" ]] || fail "cannot resolve GPU $GPU_ID UUID"
    printf 'WAITING_GPU_%s %s\n' "$GPU_ID" "$(date '+%F %T')" > "$STATUS_FILE"
    while (( idle_checks < 6 )); do
        if [[ -n "$(gpu_compute_pids "$uuid")" ]]; then
            idle_checks=0
            stamp "GPU $GPU_ID busy with PID(s) $(gpu_compute_pids "$uuid" | paste -sd, -); waiting"
        else
            ((idle_checks += 1))
            stamp "GPU $GPU_ID idle check $idle_checks/6"
        fi
        sleep "$POLL_SECONDS"
    done
}
